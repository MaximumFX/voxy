...
public class AutoBindingShader extends Shader {
    ...
    private record BufferBinding(int target, int index, GpuBuffer buffer, long offset, long size) {}
    private record TextureBinding(int unit, int sampler, GpuTexture texture) {}
    ...
    public AutoBindingShader ssboIf(String define, GpuBuffer buffer) {
    ...
    public AutoBindingShader ssbo(int index, GpuBuffer binding) {
        return this.ssbo(index, binding, 0);
    }

    public AutoBindingShader ssbo(String define, GpuBuffer binding) {
        return this.ssbo(Integer.parseInt(this.defines.get(define)), binding, 0);
    }

    public AutoBindingShader ssbo(int index, GpuBuffer buffer, long offset) {
        this.insertOrReplaceBinding(new BufferBinding(GL_SHADER_STORAGE_BUFFER, index, buffer, offset, -1));
        return this;
    }


    public AutoBindingShader ubo(String define, GpuBuffer buffer) {
        return this.ubo(Integer.parseInt(this.defines.get(define)), buffer);
    }

    public AutoBindingShader ubo(int index, GpuBuffer buffer) {
        return this.ubo(index, buffer, 0);
    }

    public AutoBindingShader ubo(int index, GpuBuffer buffer, long offset) {
    ...
    public AutoBindingShader texture(String define, GpuTexture texture) {
        return this.texture(define, -1, texture);
    }

    public AutoBindingShader texture(String define, int sampler, GpuTexture texture) {
        return this.texture(Integer.parseInt(this.defines.get(define)), sampler, texture);
    }

    public AutoBindingShader texture(int unit, int sampler, GpuTexture texture) {
        this.rebuild = true;
    ...
    public void bind() {
        ...
        if (!this.bindings.isEmpty()) {
            for (var binding : this.bindings) {
                if (binding.buffer instanceof TrackedObject tracked) {
                    tracked.assertNotFreed();
                }
        ...
        if (!this.textureBindings.isEmpty()) {
            for (var binding : this.textureBindings) {
                if (binding.texture != null) {
                    if (binding.texture instanceof TrackedObject tracked) {
                        tracked.assertNotFreed();
                    }
